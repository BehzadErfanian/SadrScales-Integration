...
**Phase:** M2 — batch API + C# Quick Start under CI validation  
...
This branch is not considered complete until its exact head commit has both SDK CI and Public Repository Guard green.
...
1. Complete CI on `m2/batch-and-csharp-sample`.
2. Fix any batch/sample failure on the branch.
3. Open PR only after branch gates are green.
4. Merge and verify exact `main` SHA.
5. Continue M2 package/release hardening: Source Link, package validation/API compatibility and strong-name decision.
